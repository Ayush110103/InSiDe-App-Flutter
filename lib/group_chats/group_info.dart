...
import 'package:inside/group_chats/add_members.dart';
import 'package:inside/screens/home_screen.dart';
...
class _GroupInfoState extends State<GroupInfo> {
  ...
  Future removeMembers(int index) async {
    ...
    // also remove the user from the group
    final CollectionReference groupsCollection =
    FirebaseFirestore.instance.collection('groups');

    try {
      // Fetch the document representing the group
      DocumentSnapshot groupDocument =
      await groupsCollection.doc(widget.groupId).get();

      if (groupDocument.exists) {
        // Retrieve the "members" array from the document data
        List<dynamic> members = groupDocument['members'];

        // Remove the user with the specific UID from the "members" array
        members.remove(uid);

        // Update the document with the modified "members" array
        await groupsCollection.doc(widget.groupId).update({'members': members});
